...
import {DeviceDAO, DeviceUpdateAliasDTO} from "../../../lib/definition";
...
DeviceManageRouter.post("/", async (req, res) => {
    try {
        ...
        if (!updateDTO.uniqueHardwareId || !updateDTO.alias) {
            res.status(400).json({
        ...
        const [result] = await db.execute<ResultSetHeader>(updateDeviceAliasQuery, [updateDTO.alias, updateDTO.uniqueHardwareId, userId]);
